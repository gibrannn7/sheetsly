    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 transition-colors">
          <div>
            <h3 id="data-quality-modal-title" className="text-sm font-bold text-slate-900 dark:text-slate-100 uppercase tracking-wide">
              {t.title}
            </h3>
        ...
        <div className="p-5 overflow-y-auto space-y-4 text-slate-700 dark:text-slate-300 text-xs">
          ...
          <div className="space-y-1">
            <h4 className="font-bold text-slate-900 dark:text-slate-100 text-xs">{t.howAssessedTitle}</h4>
            <p className="text-slate-600 dark:text-slate-400 leading-relaxed text-[11px]">{t.howAssessedDesc}</p>
          </div>

          {/* Quality Dimensions List */}
          <div className="space-y-2.5">
            <h4 className="font-bold text-slate-900 dark:text-slate-100 text-xs uppercase tracking-wider text-[10px] text-slate-500 dark:text-slate-400">
              {t.dimensionsTitle}
            </h4>
            <div className="space-y-2">
              {dimensions.map((dim, idx) => (
                <div
                  key={idx}
                  className="p-2.5 rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-950/70 space-y-1"
                >
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-slate-900 dark:text-slate-100 text-xs">{dim.title}</span>
                    <span className="text-[9px] font-mono font-bold px-1.5 py-0.5 rounded bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-300 dark:border-slate-700">
                      {dim.severity}
                    </span>
                  </div>
                  <p className="text-slate-600 dark:text-slate-400 text-[11px] leading-relaxed">{dim.desc}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="p-3 bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg space-y-1">
            <h4 className="font-bold text-slate-900 dark:text-slate-100 text-xs">{t.scoringTitle}</h4>
            <p className="text-slate-600 dark:text-slate-400 text-[11px] leading-relaxed">{t.scoringDesc}</p>
          </div>

          {/* Disclaimer / Scope Note */}
          <div className="p-3 bg-amber-50/70 dark:bg-amber-950/40 border border-amber-200/80 dark:border-amber-800/80 rounded-lg space-y-1 text-amber-900 dark:text-amber-200">
            <div className="flex items-center gap-1.5 font-bold text-xs">
              <svg className="w-3.5 h-3.5 text-amber-700 dark:text-amber-400 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                <path
                  fillRule="evenodd"
            ...
            <p className="text-[11px] leading-relaxed text-amber-800 dark:text-amber-300">{t.disclaimerText}</p>
          </div>
        </div>

...
export const SheetList: React.FC<SheetListProps> = ({
  ...
  return (
    <div className="bg-slate-100 border-b border-slate-200 px-6 py-1.5 flex items-center space-x-1.5 overflow-x-auto">
      <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 mr-2 flex-shrink-0">
        Worksheets:
      </span>
      {sheets.map((sheet) => {
        ...
        return (
          <button
            ...
            className={`flex items-center space-x-2 px-3 py-1 rounded-md text-xs font-semibold transition-all flex-shrink-0 cursor-pointer ${
              isActive
                ? 'bg-white text-slate-900 shadow-2xs border border-slate-300 ring-1 ring-slate-300/50'
                : 'text-slate-600 hover:text-slate-900 hover:bg-slate-200/70 border border-transparent'
            }`}
          >
            <svg
              className={`w-3.5 h-3.5 ${isActive ? 'text-slate-800' : 'text-slate-400'}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
